...
def progress( done, total ):
    ...
    sys.stdout.write('\rProcessing files... ' + p.ljust(progressLen) + ('\b' * (progressLen - len(p))))
...
for path, subdirs, items in os.walk(sys.argv[-1]):
    ...
    sys.stdout.write('\b\b\b' + (r'.' * (round(done) % 4)).ljust(3))
files = sorted(files)
...
unsupportedFormats = re.compile(r'^.*\.(d2s|sys|[ao])$')
...
audioFile = re.compile(r'^.+\.(m(4[bp]|p[123])|w(ma|a?v)|flac|ape)$')
videoFile = re.compile(r'.*\.(avi|flv|m(4v|[ko]v|p(4|e|e?g))|ogv|qt|rmvb|wmv)$')
buffer = []
for i in range(0, len(files)):
...
print('\r' + str(done) + ' files processed' + (r' ' * 20) + ('\b' * 20))



# CREATING MORE MEANINGFUL SUBDIRECTORIES
audioSubdir = os.path.join(sys.argv[-1], r'Audio')
try: os.mkdir(audioSubdir)
except: pass
documentsSubdir = os.path.join(sys.argv[-1], r'Documents')
try: os.mkdir(documentsSubdir)
except: pass
fontsSubdir = os.path.join(sys.argv[-1], r'Fonts')
try: os.mkdir(fontsSubdir)
except: pass
picturesSubdir = os.path.join(sys.argv[-1], r'Pictures')
try: os.mkdir(picturesSubdir)
except: pass
videosSubdir = os.path.join(sys.argv[-1], r'Videos')
try: os.mkdir(videosSubdir)
except: pass
txtSubdir = os.path.join(sys.argv[-1], r'Plain Text')
try: os.mkdir(txtSubdir)
except: pass
codeSubdir = os.path.join(sys.argv[-1], r'Code')
try: os.mkdir(codeSubdir)
except: pass
miscSubdir = os.path.join(sys.argv[-1], r'Misc')
...
picture = r'^.*\.(a(n?i|png)|b([lm]p|pg)|d(c[rx]|ds|ib)|eps[fi]?|g(d|if)|i(mt?|co|cns)|flif|vsd|heic'
picture += r'j(p[2efx]|[np]g|peg|xr)|m(ic|po|sp|ng)|p(c[dx]|ng|[abgnp]m|s[db])|odg|c(in|r2|rw|ur)|'
picture = re.compile(picture + r'[hw]dp|s(gi|vgz?)|t(ga|iff?)|w(al|ebp|mf|bmp|pg)|x([bp]m|cf))$')
...
for file in files:
    ...
    sys.stdout.write('\b\b\b' + (r'.' * (round(done / 10) % 4)).ljust(3))
...
maxFilesPerDir = 250
for subdir in [os.path.join(sys.argv[-1], d) for d in os.listdir(sys.argv[-1])]:
